...
import software.bernie.geckolib3.molang.AdvMolangParser;
...
public class BedrockComponentInitialization extends BedrockComponentBase implements IComponentEmitterInitialize, IComponentEmitterUpdate {
    ...
    public BedrockComponentBase fromJson(JsonElement elem, MolangParser parser) throws MolangException {
        ...
        if (element.has("creation_expression"))
            this.creation = ((AdvMolangParser) parser).parseGlobalJson(element.get("creation_expression"));
        if (element.has("per_update_expression"))
            this.update = ((AdvMolangParser) parser).parseGlobalJson(element.get("per_update_expression"));
        if (element.has("particle_update_expression"))
            this.particleUpdate = ((AdvMolangParser) parser).parseGlobalJson(element.get("particle_update_expression"));
